...
export class GoogleDriveService {
  ...
  // =====================================
  // FILE LISTING / METADATA (thin wrappers — reusable, typed)
  // =====================================
  ...
  listFiles(query: string, pageToken?: string): Promise<DriveFileList> {
    ...
    const params: Record<string, string> = {
      q: query,
      fields: 'nextPageToken, files(id, name, mimeType, parents, size, createdTime, modifiedTime, webViewLink, webContentLink, trashed)',
      pageSize: '50'
    };
  ...
  getFile(fileId: string): Promise<DriveFile> {
    return this.get<DriveFile>(`/files/${fileId}`, {
      fields: 'id, name, mimeType, parents, size, createdTime, modifiedTime, webViewLink, webContentLink, trashed'
    });
  }
  ...
  uploadMultipart(
    ...
    return this.withAuthRetry(async (accessToken) => {
      ...
      return this.xhrRequest<DriveFile>({
        method: 'POST',
        url: `${UPLOAD_BASE}/files?uploadType=multipart&fields=id,name,mimeType,parents,size,createdTime,modifiedTime,webViewLink,webContentLink`,
        accessToken,
        body,
  ...
  // =====================================
  // CORE REQUEST (JSON, retry + refresh aware)
  // =====================================
    ...
    return this.withAuthRetry(async (accessToken) => {
      ...
      while (true) {
        ...
        try {
          ...
          if (res.status === 401) {
            // let withAuthRetry's caller handle refresh — signal via throw
          ...
          if (err instanceof DriveAuthError) {
            throw err; // handled by withAuthRetry
          }
          ...
          // network-level failure (fetch threw) — retry if we have attempts left
  ...
  // =====================================
  // AUTH WRAPPER — injects token, refreshes once on 401, then gives up
  // =====================================
  ...
  // =====================================
  // XHR (needed for upload/download progress — fetch can't report upload progress)
  // =====================================
...
private xhrRequest<T>(opts: {
  ...
  url: string;
  accessToken: string;
  body?: XMLHttpRequestBodyInit;   // was: BodyInit
  contentType?: string;
